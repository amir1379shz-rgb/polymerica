...
export default function PolymerMarket() {
  ...
  return (
    <div className="min-h-full">
      ...
      <header className="sticky top-0 z-30 pm-panel backdrop-blur border-b pm-line">
        <div className="max-w-6xl mx-auto px-4 md:px-6 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowMore(true)} className="pm-muted"><Icon name="menu" size={22} /></button>
          </div>
        </div>
      </header>
      ...
      {tab === "post" && <PostAd form={form} setForm={setForm} onSubmit={submitAd} error={formError} myCount={myCount} gate={gate} setGate={setGate} goPricing={() => setTab("pricing")} user={user} openLogin={() => setShowLogin(true)} isPaid={isPaid} editingId={editingId} onCancelEdit={cancelEdit} />}
  ...
  return (
    <main>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pb-10">
        <a href="/prices.html" className="group flex items-center justify-between gap-4 rounded-2xl px-6 py-5 md:px-8 md:py-6" style={{background:"linear-gradient(90deg,#1B3358 0%,#12233F 100%)"}}>
          <div className="flex items-center gap-4">
            <span className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{background:"rgba(169,129,47,0.18)"}}>
              <Icon name="trendingUp" size={22} className="pm-gold" />
            </span>
            <div>
...
function ListingCard({ l, onClick }) {
  ...
  return (
    <div onClick={onClick} className={`pm-panel pm-card rounded-xl overflow-hidden cursor-pointer flex flex-row sm:flex-col ${l.featured ? "ring-2 ring-[#A9812F]" : ""}`}>
        ...
        {images ? (
          <div>
            <div className="h-56 relative bg-black/5">
              ...
              <button onClick={onClose} className="absolute top-3 left-3 bg-white/80 rounded-full p-1.5"><Icon name="x" size={16} /></button>
              {images.length > 1 && (
                <>
                  <button onClick={() => setActiveImg(i => (i - 1 + images.length) % images.length)} className="absolute top-1/2 -translate-y-1/2 right-2 bg-white/80 rounded-full p-1.5"><Icon name="chevronRight" size={16} /></button>
                  <button onClick={() => setActiveImg(i => (i + 1) % images.length)} className="absolute top-1/2 -translate-y-1/2 left-2 bg-white/80 rounded-full p-1.5"><Icon name="chevronLeft" size={16} /></button>
...
function PostAd({ form, setForm, onSubmit, error, myCount, gate, setGate, goPricing, user, openLogin, isPaid, editingId, onCancelEdit }) {
      ...
      {gate ? (
        ...
        <form onSubmit={onSubmit} className="pm-panel rounded-xl p-5 md:p-6 grid md:grid-cols-2 gap-4overflow-hidden">
          ...
          <div className="md:col-span-2 flex justify-end">
            <button type="submit" className="pm-btn-primary rounded-lg px-6 py-3 text-sm flex items-center gap-2">
