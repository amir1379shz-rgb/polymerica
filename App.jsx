...
const PLANS = [
  ...
  { id: "pro", name: "حرفه‌ای", price: 490000, priceLabel: "۴۹۰,۰۰۰", period: "تومان / ماه", features: ["آگهی نامحدود", "نشان «مشترک ویژه» روی آگهی‌ها", "اولویت نمایش در نتایج", "مشاهده نامحدود اطلاعات تماس", "آمار بازدید آگهی‌ها"], cta: "خرید اشتراک حرفه‌ای", highlight: true },
...
const RULES = [
  ...
  "نشان «مشترک ویژه» صرفاً نشان‌دهنده تهیه اشتراک پولی است و به‌منزله احراز هویت، ضمانت کیفیت یا اصالت کالا نیست.",
...
export default function PolymerMarket() {
  ...
  useEffect(() => {
    const titles = {
      home: "بازار پلیمر | سامانه رسمی بازار پلیمر و پلاستیک",
      market: "بازار آگهی‌های خرید و فروش پلیمر | بازار پلیمر",
      post: "ثبت آگهی خرید یا فروش | بازار پلیمر",
      my: "آگهی‌های من | بازار پلیمر",
      pricing: "تعرفه اشتراک | بازار پلیمر",
      rules: "قوانین و مقررات | بازار پلیمر",
      about: "درباره سامانه | بازار پلیمر",
    };
    document.title = titles[tab] || "بازار پلیمر";
  }, [tab]);
      ...
      <header className="sticky top-0 z-30 pm-panel backdrop-blur border-b pm-line">
        <div className="max-w-6xl mx-auto px-4 md:px-6 flex items-center justify-between h-16">
          <button onClick={() => setTab("home")} className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="font-extrabold text-lg tracking-tight pm-navy">بازار پلیمر</div>
          </button>
    ...
    <main>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pb-16 grid md:grid-cols-3 gap-4">
        {[
          ...
          ["shieldCheck","معاملات مطمئن‌تر","فروشندگان مشترک ویژه با نشان مخصوص روی آگهی‌هاشون مشخص می‌شوند."],
...
function ListingCard({ l, onClick }) {
  ...
  return (
    <div onClick={onClick} className={`pm-panel pm-card rounded-xl overflow-hidden cursor-pointer flex flex-row-reverse sm:flex-col ${l.featured ? "pm-ring-gold" : ""}`}>
      ...
      <div className="pm-panel rounded-xl w-full max-w-lg max-h-[88vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
        ...
        <div className="p-5 flex flex-col gap-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className={`stamp shrink-0 ${listing.verified ? "stamp-green" : "stamp-gray"}`}>{listing.verified ? "مشترک ویژه" : "حساب پایه"}</div>
          </div>
...
function PostAd({ form, setForm, onSubmit, error, myCount, gate, setGate, goPricing, user, openLogin, isPaid, editingId, onCancelEdit, submitting }) {
  ...
  const handleImages = async (e) => {
    ...
    try {
      ...
      const ready = await Promise.all(picked.map(async (f) => {
        const isHeic = /heic|heif/i.test(f.type) || /\.(heic|heif)$/i.test(f.name || "");
        if (!isHeic) return f;
        // مرورگر گوشی نمی‌تواند HEIC را مستقیم بخواند، پس اینجا با یک کتابخانه به JPEG تبدیلش می‌کنیم
        const heic2any = (await import("heic2any")).default;
        const result = await heic2any({ blob: f, toType: "image/jpeg", quality: 0.8 });
        const blob = Array.isArray(result) ? result[0] : result;
        return new File([blob], (f.name || "image").replace(/\.(heic|heif)$/i, "") + ".jpg", { type: "image/jpeg" });
      }));
      const uploaded = await Promise.all(ready.map(f => uploadListingImage(f, 1000, 0.72)));
      ...
      else if (msg === "bad-image") setImgErr("این تصویر با فرمتی ذخیره شده که مرورگر نمی‌تواند بازش کند. در تنظیمات دوربین گوشی، فرمت عکس را روی JPEG بگذارید و دوباره امتحان کنید.");
      else if (msg === "upload-failed") setImgErr("بارگذاری تصویر روی سرور ناموفق بود؛ اتصال اینترنت را بررسی کنید و دوباره امتحان کنید.");
      else setImgErr("تبدیل یا بارگذاری تصویر ناموفق بود؛ لطفاً دوباره امتحان کنید یا یک عکس JPG/PNG امتحان کنید.");
    }
    setImgBusy(false);
  };
    ...
    <main className="max-w-3xl mx-auto px-4 md:px-6 py-10">
      ...
      {gate ? (
        <div className="pm-panel rounded-xl p-8 text-center">
          ...
          <p className="pm-muted text-sm mb-6 max-w-md mx-auto leading-7">برای ثبت آگهی‌های بیشتر و دریافت نشان «مشترک ویژه»، به پلن حرفه‌ای یا عمده ارتقا دهید.</p>
        ...
        <form onSubmit={onSubmit} className="pm-panel rounded-xl p-5 md:p-6 grid md:grid-cols-2 gap-4 overflow-hidden">
          ...
          <Field label={`تصاویر محصول (${(form.images||[]).length}/۴)`} full>
            ...
            <div className="pm-muted text-[11px]">حداکثر ۴ عکس؛ عکس‌های HEIC آیفون هم پشتیبانی می‌شود (خودکار تبدیل می‌شود). برای نمایش بهتر، عکس واضح و نورگیر از محصول ثبت کنید.</div>
          </Field>
          ...
          {listings.map(l => (
            <div key={l.id} className="pm-panel rounded-xl p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                <span className={`stamp stamp-sm ${l.verified ? "stamp-green" : "stamp-gray"}`}>{l.verified ? "مشترک ویژه" : "پایه"}</span>
              </div>
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 md:px-6 py-10">
      <div className="mb-8 text-center max-w-xl mx-auto">
        ...
        <p className="pm-muted text-sm leading-7">با ارتقا به پلن‌های حرفه‌ای، نشان «مشترک ویژه» دریافت کنید و آگهی‌های خود را در اولویت دید خریداران قرار دهید.</p>
      </div>
...
function Privacy() {
  return (
    <main className="max-w-3xl mx-auto px-4 md:px-6 py-10">
      ...
      <p className="pm-muted text-sm mb-6">سامانه بازار پلیمر متعهد به حفظ اطلاعات شخصی کاربران است.</p>
...
function About() {
  return (
    <main className="max-w-3xl mx-auto px-4 md:px-6 py-10">
      <h1 className="text-2xl font-bold mb-2">درباره بازار پلیمر</h1>
      ...
      <div className="pm-panel rounded-xl p-6 leading-8 text-sm flex flex-col gap-4">
        <p>
          بازار پلیمر با هدف تسهیل ارتباط مستقیم میان تولیدکنندگان، بازرگانان، واحدهای بازیافت و کارخانجات صنایع تکمیلی پلاستیک راه‌اندازی شده است.
        </p>
        <p>
    ...
    <footer className="pm-panel border-t pm-line mt-20 pb-20 md:pb-8 pt-10">
      <div className="max-w-6xl mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-6 text-xs pm-muted">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold pm-navy text-sm">بازار پلیمر</span>
